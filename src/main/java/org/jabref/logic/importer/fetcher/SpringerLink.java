...
public class SpringerLink implements FulltextFetcher {
    ...
    public Optional<URL> findFullText(BibEntry entry) throws IOException {
        ...
        if (doi.isPresent()) {
            ...
            JsonObject json = client.target(API_URL)
                    .queryParam("api_key", API_KEY)
                    .queryParam("q", String.format("doi:%s", doi.get().getDOI()))
                    .request()
                    .get(JsonObject.class);

            int results = json.getAsJsonArray("result").get(0).getAsJsonObject().get("total").getAsInt();
